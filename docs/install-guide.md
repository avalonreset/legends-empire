# Install Legends Obsidian

Legends Obsidian, based on Daniel Agrici's claude-obsidian, has two independent parts:
...
claude plugin install legends-obsidian@avalonreset-legends-obsidian
claude plugin list
```

This public source tree includes the audited marketplace catalog. Development
checkouts containing contributor-vault state must not be added as marketplaces.
The release builder injects and validates the catalog in its clean artifact;
public source is promoted from that artifact.
...
The installer defaults to a no-write preview for Grok, Codex, Gemini, Claude, and OpenCode. Select one host explicitly when you only want that integration:
...
```text
Grok:      ~/.grok/skills/<name>            -> <product-repository>/skills/<name>
Claude:    ~/.claude/skills/<name>          -> <product-repository>/skills/<name>
...
## Native Windows skill installation

Use Python 3.11+ and PowerShell; this changes skill discovery only:

```powershell
pwsh -File bin/setup-multi-agent.ps1 -Hosts grok,codex,gemini,claude
pwsh -File bin/setup-multi-agent.ps1 -Hosts grok,codex,gemini,claude -Mode apply
pwsh -File bin/setup-multi-agent.ps1 -Hosts grok,codex,gemini,claude -Mode check
```

The installer uses per-skill junctions, previews by default, and refuses a
conflict without replacing existing paths. `-HomeDirectory` selects an isolated
home for testing. Bash supports the equivalent `--home` option. Workspace
installation uses `--scope workspace --workspace PATH` (PowerShell:
`-Scope workspace -Workspace PATH`). These options never select a user vault.

For Claude, use either flat skills or its plugin to avoid duplicate discovery.
The plugin retains upstream's opt-in Claude hooks; portable skill installation
does not emulate those hooks in other hosts. Restart the host after installation.

Protected vault writes and first-time retrieval-index creation still need WSL;
review and apply inside the same WSL environment. Read
[the compatibility matrix](AGENTS-MATRIX.md) for evidence and limits.
...
```bash
claude plugin uninstall legends-obsidian@avalonreset-legends-obsidian
claude plugin marketplace remove avalonreset-legends-obsidian
```
