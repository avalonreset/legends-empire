...
class ReleaseRepo(unittest.TestCase):
    ...
    def commit(self, message: str) -> None:
        self.git("add", "-A")
        if (self.repo / "src/tool.py").exists():
            self.git("update-index", "--chmod=+x", "src/tool.py")
...
class CanonicalPolicyTests(unittest.TestCase):
    ...
    def test_repository_allowlist_is_canonical_and_valid(self) -> None:
        ...
        self.assertFalse(_selected("_templates/example.md", config))
...
class AuditTests(ReleaseRepo):
    ...
    def test_binary_control_bytes_survive_archive_snapshot(self) -> None:
        archive = self.home / "binary-payload.zip"
        self._write_zip(archive, [("README.md", b"before\x1aafter\r\n")])
        report = audit_artifact(archive)
        # A non-product ZIP can fail policy, but it must not be truncated by
        # Windows text-mode Ctrl-Z handling before structural validation.
        self.assertNotIn("invalid_archive", _codes(report))
        self.assertNotIn("unreadable_artifact", _codes(report))
