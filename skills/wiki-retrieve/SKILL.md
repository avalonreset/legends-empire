...
For a mature vault whose canonical knowledge also lives outside `wiki/`, add `--all-vault`. This scans the canonical Markdown spine while excluding hidden state, `inbox/assets`, and `meta/history`:

```bash
python3 "$PREFIX" --vault "$VAULT" --all-vault --no-llm --peek
bash "$PRODUCT_ROOT/scripts/setup-retrieve.sh" --vault "$VAULT" --no-llm --all-vault --apply
```
