...
Muse joins as a P0 host. No upstream change; upstream pin stays
`AgriciDaniel/claude-obsidian` v2.2.0.

### Added

- `scripts/setup-multi-agent.sh --host muse`: per-skill symlinks into
  `~/.agents/skills` (shared user-skill root, Codex-compatible).
- Muse rows in `docs/AGENTS-MATRIX.md` support tiers, host table, and
  verify table. Router skill lists Muse as agent-agnostic.
- Capability contract for the `legends-obsidian` brand router in
  `config/capabilities.json` (tier core, transaction none). Closes the
  `unregistered_skill` contracts error.

## [2.2.0] - 2026-09-11 (Legends Obsidian house cut)

House speech cut of upstream `AgriciDaniel/claude-obsidian` **v2.2.0**
(`32ac5a02c4e082e4a5628ca810776375e134708e`, zip SHA-256
`6207f8aff60366adb441f4311e542fed42ef1edec1b303e78ee215cec8b64ffa`).

### Added

- Spoken brand **Legends Obsidian** (`LEGENDS.md`, `skills/legends-obsidian`,
  `docs/AGENTS-MATRIX.md`, `.codex-plugin/plugin.json`).
- Windows installer `bin/setup-multi-agent.ps1` (also copied to `scripts/`)
...
- Lint link resolution no longer reports a wikilink as ambiguous when the
  ...
  reads only `.gitignore` files inside the vault root, excluding
