...
WHOLE_VAULT_ROOT_FILES = ("Home.md", "NOW.md", "AGENTS.md", "CODEX.md")
WHOLE_VAULT_DIRS = (
    "areas", "decisions", "fleet", "inbox", "meta", "people",
    "projects", "sops", "wiki",
)
WHOLE_VAULT_EXCLUDED_PREFIXES = (
    "inbox/assets/",
    "meta/history/",
)


def is_canonical_vault_page(relative: str) -> bool:
    rel = relative.replace("\\", "/")
    if rel in WHOLE_VAULT_ROOT_FILES:
        return True
    if any(rel.startswith(prefix) for prefix in WHOLE_VAULT_EXCLUDED_PREFIXES):
        return False
    first = rel.split("/", 1)[0]
    return first in WHOLE_VAULT_DIRS
...
def collect_pages(target, all_vault=False):
    if all_vault:
        pages = []
        for name in WHOLE_VAULT_ROOT_FILES:
            candidate = VAULT_ROOT / name
            if candidate.is_file() and not candidate.is_symlink():
                pages.append(candidate)
        for name in WHOLE_VAULT_DIRS:
            base = VAULT_ROOT / name
            if not base.is_dir():
                continue
            for page in base.rglob("*.md"):
                try:
                    rel_path = page.relative_to(VAULT_ROOT)
                except ValueError:
                    continue
                ...
                if page.is_symlink():
                    continue
                if not is_canonical_vault_page(rel):
                    continue
                pages.append(page)
...
def process_selection(args, pages, *, root_fd=None, meta_fd=None):
    ...
    if not preflight_unique_addresses(
        collect_pages("--all", all_vault=getattr(args, "all_vault", False)),
        root_fd=root_fd,
    ):
    ...
    if args.path == "--all" or getattr(args, "all_vault", False):
...
def main():
    ...
    parser.add_argument(
        "--all-vault",
        action="store_true",
        help="Process the canonical whole-vault Markdown spine "
        "(areas, decisions, fleet, inbox, meta, people, projects, "
        "sops, wiki, and root entry notes). Excludes hidden state, "
        "inbox/assets, and meta/history.",
    )
    parser.add_argument(
        "--no-llm",
        action="store_true",
    ...
    if getattr(args, "all_vault", False) and (args.path or args.all):
        parser.error("--all-vault cannot be combined with a path or --all")
    if args.all and not args.path:
        args.path = "--all"
    elif not args.path and not getattr(args, "all_vault", False):
        # No path and no --all: default to all (matches the help text)
        args.path = "--all"

    pages = collect_pages(args.path, all_vault=getattr(args, "all_vault", False))
    ...
    if args.path != "--all" and not getattr(args, "all_vault", False):
